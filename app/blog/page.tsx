...
import { BLOG_CATEGORIES, FIRST_PAGE_SIZE } from "@/lib/blog-card";
...
/**
 * The blog index (/blog/). Same archive template as the category pages, but
 * lists every post across categories with the "All" pill active. Previously
 * this URL was proxied to WordPress, which canonical-redirected any request
 * whose forwarded host was not creditdanny.com — so on localhost (and any
 * preview deployment) it bounced to the live site.
 */
export const revalidate = 3600;

const CANONICAL = "https://creditdanny.com/blog/";
const TITLE = "Credit Danny Blog | Expert Homebuying & Financial Advice";
const DESCRIPTION =
  "Discover actionable credit repair strategies on our blog. Credit Danny posts resources that guide you through mortgage preparation steps. Check out our blog.";
...
export default async function BlogIndex() {
  const [page, categories] = await Promise.all([
    getPostsPage({ offset: 0, perPage: FIRST_PAGE_SIZE }),
    getAllCategories(),
  ]);
  const byId = new Map(categories.map((c) => [c.id, c]));
  const posts = page.posts.map((p) => toCardPost(p, categoryLabel(p, byId)));
  ...
  return (
    <>
      ...
      <meta name="twitter:data1" content={String(page.total)} />
        ...
        <div id="archive" style={{ position: "relative" }}>
          ...
          {featured && (
            <div className="cdb-cards-pad" style={{ padding: "0px 60px", marginTop: -110, position: "relative" }}>
              <div className="cdb-feat-wrap" style={{ background: "rgb(255, 255, 255)", borderRadius: 25, boxShadow: "rgba(0, 0, 0, 0.1) 0px 0px 10px", display: "grid", gridTemplateColumns: "minmax(300px, 1.05fr) minmax(380px, 1fr)", overflow: "hidden" }}>
                <Link className="cdb-feat-link" href={featured.href} style={{ display: "block", textDecoration: "none" }} aria-label={featured.title}>
                  <div className="cdb-feat-img" style={{ background: `url(${featured.imageLarge ?? ""}), linear-gradient(160deg, rgb(0, 6, 58), rgb(0, 173, 238)) 0% 0% / cover`, backgroundSize: "cover", backgroundPosition: "center", minHeight: 320 }} />
                </Link>
                <div style={{ padding: "44px 44px 40px" }}>
                  <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                    ...
                    <span style={{ background: "rgb(234, 245, 255)", color: "rgb(12, 112, 195)", borderRadius: 999, padding: "6px 16px", fontSize: 12, fontWeight: 700, letterSpacing: 0.6 }}>{featured.label}</span>
                  </div>
                  <div style={{ fontFamily: '"Podium Sharp", Impact, sans-serif', fontSize: 44, lineHeight: 1.06, textTransform: "uppercase", color: "rgb(0, 0, 0)", marginTop: 20, textWrap: "pretty" }}>
                    <Link href={featured.href} style={{ color: "inherit", textDecoration: "none" }}>{featured.title}</Link>
                  </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 26, fontSize: 13, fontWeight: 600, color: "rgb(90, 90, 90)" }}>
                    ...
                    <span>{featured.longDate}</span>
                    <span style={{ color: "rgb(217, 217, 217)" }}>•</span>
                    <span>{featured.minutes} min read</span>
                  </div>
                  <div style={{ display: "flex", marginTop: 26 }}>
                    <Link href={featured.href} style={{ fontFamily: "var(--font-display)", fontWeight: 750, textTransform: "uppercase", letterSpacing: 0.2, cursor: "pointer", textDecoration: "none", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 16, padding: "9px 18px", borderRadius: 8, background: "rgb(0, 0, 0)", color: "rgb(255, 255, 255)" }}>
                      Read the article
                    </Link>
                  </div>
                </div>
          ...
          <div style={{ padding: "56px 60px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div className="cdb-pills" style={{ display: "flex", gap: 10 }}>
              <Link href="/blog/" style={PILL_ALL} aria-current="page">All</Link>
              {BLOG_CATEGORIES.map((c) => (
                <Link key={c.slug} href={`/category/${c.slug}/`} style={PILL_INACTIVE}>{c.name}</Link>
              ))}
            </div>
          </div>

          <PostGrid initialPosts={gridPosts} initialOffset={posts.length} total={page.total} />
