...
import { BLOG_CATEGORIES, type CardPost } from "@/lib/blog-card";
import type { WpCategory } from "@/lib/wp";

export const SITE = "https://creditdanny.com";

export function categoryCanonical(slug: string): string {
  return `${SITE}/category/${slug}/`;
}

export function categoryTitle(category: WpCategory): string {
  return `${category.name} Archives | Credit Danny`;
}

function jsonLd(category: WpCategory): string {
  const url = categoryCanonical(category.slug);
    ...
    "@graph": [
      {
        ...
        "@id": `${SITE}/#person`,
        name: "Credit Danny",
        sameAs: ["https://www.instagram.com/creditdanny/", "https://www.tiktok.com/@creditdanny"],
        image: {
          "@type": "ImageObject",
          "@id": `${SITE}/#logo`,
          url: `${SITE}/wp-content/uploads/2025/03/credit-danny-logo-black2.png`,
          contentUrl: `${SITE}/wp-content/uploads/2025/03/credit-danny-logo-black2.png`,
          caption: "Credit Danny",
          inLanguage: "en-US",
          width: "1394",
          height: "261",
        },
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: "2", item: { "@id": url, name: category.name } },
        ],
      },
      {
        "@type": "CollectionPage",
...
type Props = {
  category: WpCategory;
  /** First page of the category, newest first (featured post first). */
  posts: CardPost[];
  /** Total posts in the category, from WordPress. */
  total: number;
};

/**
 * The category archive template for /category/<slug>/. Same layout as the
 * blog index, scoped to one category with its pill active. The card grid is a
 * client component that fetches further batches from WordPress on demand.
 */
export default function CategoryArchive({ category, posts, total }: Props) {
  ...
  const bodyClass =
    ...
    " wp-custom-logo wp-embed-responsive wp-theme-hello-elementor cd-blog cd-blog-archive eio-default hello-elementor-default elementor-default elementor-kit-27255861";
  ...
  return (
    <>
      ...
      <script type="application/ld+json" className="rank-math-schema" dangerouslySetInnerHTML={{ __html: jsonLd(category) }} />
        ...
        <div id="archive" style={{ position: "relative" }}>
              ...
              <div className="cdb-feat-wrap" style={{ background: "rgb(255, 255, 255)", borderRadius: 25, boxShadow: "rgba(0, 0, 0, 0.1) 0px 0px 10px", display: "grid", gridTemplateColumns: "minmax(300px, 1.05fr) minmax(380px, 1fr)", overflow: "hidden" }}>
                ...
                <div style={{ padding: "44px 44px 40px" }}>
                  <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                    ...
                    <span style={{ background: "rgb(234, 245, 255)", color: "rgb(12, 112, 195)", borderRadius: 999, padding: "6px 16px", fontSize: 12, fontWeight: 700, letterSpacing: 0.6 }}>{label}</span>
                  </div>
          ...
          <div style={{ padding: "56px 60px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div className="cdb-pills" style={{ display: "flex", gap: 10 }}>
              <Link href="/blog/" style={PILL_INACTIVE}>All</Link>
              {BLOG_CATEGORIES.map((c) => (
                <Link
                  key={c.slug}
                  href={`/category/${c.slug}/`}
                  style={c.slug === category.slug ? PILL_ACTIVE : PILL_INACTIVE}
          ...
          <PostGrid initialPosts={gridPosts} initialOffset={posts.length} total={total} categoryId={category.id} />
